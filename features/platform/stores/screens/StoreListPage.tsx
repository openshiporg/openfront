/**
 * StoreListPage - Server Component
 * Uses dedicated Stores actions for consistent data fetching
 */

import { getListByPath } from '../../../dashboard/actions/getListByPath'
import { getAdminMetaAction } from '../../../dashboard/actions'
import { notFound } from 'next/navigation'
import { StoreListPageClient } from './StoreListPageClient'
import { getFilteredStores, getStoreStatusCounts } from '../actions'

interface PageProps {
  searchParams: Promise<{ [key: string]: string | string[] | undefined }>;
}

export async function StoreListPage({ searchParams }: PageProps) {
  ...
  // Hardcode the list key for stores
  const listKeyPath = 'stores';
  ...
  // Use dedicated Stores actions
  const response = await getFilteredStores(
  ...
  const statusCountsResponse = await getStoreStatusCounts()
