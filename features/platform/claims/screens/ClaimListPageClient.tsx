/**
 * ClaimListPageClient - Client Component  
 * Based on dashboard ListPageClient but hardcoded for claims
 */
...
interface ClaimListPageClientProps {
  list: any
...
function EmptyStateDefault() {
  return (
    <EmptyState
      ...
      description="You can create a new claim to get started."
...
export function ClaimListPageClient({
  list,
  initialData,
  initialError,
  initialSearchParams,
  statusCounts
}: ClaimListPageClientProps) {
  ...
  // For claims, use raw GraphQL string to include relationship fields
  ...
  const breadcrumbs = [
    ...
    { type: 'page' as const, label: 'Claims' }
  ]
  ...
  return (
    <PageContainer title="Claims" header={header} breadcrumbs={breadcrumbs}>
      {/* Filter Bar - includes search, filters, sorting, and create button */}
      <div className="px-4 md:px-6">
        <PlatformFilterBar list={list} />
      </div>

      {/* Note: ClaimOrder doesn't have status field, so no StatusTabs */}
        ...
        <>
          ...
          <div className="grid grid-cols-1 divide-y">
            {data?.items?.map((claim: any) => (
              <ClaimDetailsComponent key={claim.id} claim={claim} list={list} />
            ))}
          </div>
          ...
          {data && data.count > pageSize && (
            <div className="px-4 md:px-6 py-4">
              <Pagination
                ...
                total={data.count}
                pageSize={pageSize}
                list={{ singular: 'claim', plural: 'claims' }}
              />
            </div>
          )}
        </>
