/**
 * ShippingProviderListPageClient - Client Component  
 * Based on dashboard ListPageClient but hardcoded for shippingproviders
 */
...
import { ShippingProviderDetailsComponent } from '../components/ShippingProviderDetailsComponent'
import { Pagination } from '../../../dashboard/components/Pagination'
import { FilterList } from '../../../dashboard/components/FilterList'
import { CreateShippingProviderDrawer } from '../components/drawers/CreateShippingProviderDrawer'
...
interface ShippingProviderListPageClientProps {
  list: any
...
function EmptyStateDefault() {
  return (
    <EmptyState
      title="No Shipping Providers Created"
      description="You can create a new shipping provider to get started."
...
export function ShippingProviderListPageClient({
  list,
  initialData,
  initialError,
  initialSearchParams,
  regionCounts
}: ShippingProviderListPageClientProps) {
  ...
  // For shipping-providers, use raw GraphQL string to include relationship fields (from working repomix)
  ...
  const breadcrumbs = [
    ...
    { type: 'page' as const, label: 'ShippingProviders' }
  ]
  ...
  const header = (
    <div>
      ...
      <p className="text-muted-foreground">
        Create and manage shippingproviders
      </p>
    </div>
  )
  ...
  return (
    <>
    <PageContainer title="ShippingProviders" header={header} breadcrumbs={breadcrumbs}>
      ...
      {error ? (
        ...
        <>
          ...
          <div className="grid grid-cols-1 divide-y">
            {data?.items?.map((shippingprovider: any) => (
              <ShippingProviderDetailsComponent key={shippingprovider.id} shippingprovider={shippingprovider} list={list} />
            ))}
          </div>
          ...
          {data && data.count > pageSize && (
            <div className="px-4 md:px-6 py-4">
              <Pagination
                ...
                total={data.count}
                pageSize={pageSize}
                list={{ singular: 'shippingprovider', plural: 'shippingproviders' }}
              />
            </div>
          )}
        </>
    ...
    {/* Create Shipping Provider Drawer */}
    <CreateShippingProviderDrawer
