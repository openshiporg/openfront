/**
 * ShippingProviderListPage - Server Component
 * Uses dedicated ShippingProviders actions for consistent data fetching
 */

import { getListByPath } from '../../../dashboard/actions/getListByPath'
import { getAdminMetaAction } from '../../../dashboard/actions'
import { notFound } from 'next/navigation'
import { ShippingProviderListPageClient } from './ShippingProviderListPageClient'
import { getFilteredShippingProviders, getShippingProviderStatusCounts } from '../actions'

interface PageProps {
  searchParams: Promise<{ [key: string]: string | string[] | undefined }>;
}

export async function ShippingProviderListPage({ searchParams }: PageProps) {
  ...
  // Hardcode the list key for shippingproviders
  const listKeyPath = 'shipping-providers';
  ...
  // Use dedicated ShippingProviders actions
  const response = await getFilteredShippingProviders(
  ...
  const statusCountsResponse = await getShippingProviderStatusCounts()
  ...
  return (
    <ShippingProviderListPageClient
      list={enhancedList}
