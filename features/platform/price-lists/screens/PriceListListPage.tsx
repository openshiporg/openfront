/**
 * PriceListListPage - Server Component
 * Uses dedicated PriceLists actions for consistent data fetching
 */

import { getListByPath } from '../../../dashboard/actions/getListByPath'
import { getAdminMetaAction } from '../../../dashboard/actions'
import { notFound } from 'next/navigation'
import { PriceListListPageClient } from './PriceListListPageClient'
...
export async function PriceListListPage({ searchParams }: PageProps) {
  ...
  // Hardcode the list key for pricelists
  const listKeyPath = 'price-lists';
  ...
  // Use dedicated PriceLists actions
  const response = await getFilteredPriceLists(
  ...
  const statusCountsResponse = await getPriceListStatusCounts()
