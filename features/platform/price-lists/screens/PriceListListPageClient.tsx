/**
 * PriceListListPageClient - Client Component  
 * Based on dashboard ListPageClient but hardcoded for pricelists
 */
...
interface PriceListListPageClientProps {
  list: any
...
function EmptyStateDefault() {
  return (
    <EmptyState
      title="No Price Lists Created"
      description="You can create a new price list to get started."
...
export function PriceListListPageClient({
  list,
  initialData,
  initialError,
  initialSearchParams,
  statusCounts
}: PriceListListPageClientProps) {
  ...
  // For price-lists, use raw GraphQL string to include relationship fields
  ...
  const breadcrumbs = [
    ...
    { type: 'page' as const, label: 'PriceLists' }
  ]
  ...
  return (
    <>
    <PageContainer title="PriceLists" header={header} breadcrumbs={breadcrumbs}>
      ...
      {error ? (
        ...
        <>
          ...
          <div className="grid grid-cols-1 divide-y">
            {data?.items?.map((pricelist: any) => (
              <PriceListDetailsComponent key={pricelist.id} pricelist={pricelist} list={list} />
            ))}
          </div>
          ...
          {data && data.count > pageSize && (
            <div className="px-4 md:px-6 py-4">
              <Pagination
                ...
                total={data.count}
                pageSize={pageSize}
                list={{ singular: 'pricelist', plural: 'pricelists' }}
              />
            </div>
          )}
        </>
