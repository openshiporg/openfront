/**
 * PaymentProviderListPage - Server Component
 * Uses dedicated PaymentProviders actions for consistent data fetching
 */

import { getListByPath } from '../../../dashboard/actions/getListByPath'
import { getAdminMetaAction } from '../../../dashboard/actions'
import { notFound } from 'next/navigation'
import { PaymentProviderListPageClient } from './PaymentProviderListPageClient'
import { getFilteredPaymentProviders, getPaymentProviderStatusCounts } from '../actions'

interface PageProps {
  searchParams: Promise<{ [key: string]: string | string[] | undefined }>;
}

export async function PaymentProviderListPage({ searchParams }: PageProps) {
  ...
  // Hardcode the list key for paymentproviders
  const listKeyPath = 'payment-providers';
  ...
  // Use dedicated PaymentProviders actions
  const response = await getFilteredPaymentProviders(
  ...
  const statusCountsResponse = await getPaymentProviderStatusCounts()
