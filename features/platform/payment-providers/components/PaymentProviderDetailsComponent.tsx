                ...
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <span className="text-sm font-medium">
                    <span className="text-muted-foreground/75">
                      {new Date(paymentprovider.createdAt).toLocaleDateString("en-US", {
                        year: "numeric",
                        month: "long",
                        day: "numeric",
                      })}
            ...
            <div className="flex flex-col justify-between h-full">
              <div className="flex items-center gap-2">
                {paymentprovider.status && (
                  <Badge
                    color={
                      statusColors[paymentprovider.status as keyof typeof statusColors] ||
                      "zinc"
                    }
                    className="text-[.6rem] sm:text-[.7rem] py-0 px-2 sm:px-3 tracking-wide font-medium rounded-md border h-6"
                  >
                    {paymentprovider.status.toUpperCase()}
                  </Badge>
                )}
              ...
              <div className="px-4 md:px-6 py-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                  <div>
                    ...
                    <span className="ml-2 font-medium">{paymentprovider.id}</span>
                  </div>
                  {paymentprovider.updatedAt && (
                    <div>
                      <span className="text-muted-foreground">Updated:</span>
                      <span className="ml-2 font-medium">
                        {new Date(paymentprovider.updatedAt).toLocaleDateString()}
                      </span>
                    </div>
                  )}
                </div>
