...
export function ReturnDetailsComponent({
  return,
  list,
}: ReturnDetailsComponentProps) {
  const [isEditDrawerOpen, setIsEditDrawerOpen] = useState(false);

  return (
    <>
      <Accordion type="single" collapsible className="w-full">
        <AccordionItem value={return.id} className="border-0">
            ...
            <div className="flex items-start gap-4">
              ...
              <div className="flex flex-col items-start text-left gap-2 sm:gap-1.5">
                <div className="flex flex-wrap items-center gap-2">
                  <Link
                    href={`/dashboard/platform/returns/${return.id}`}
                  ...
                  <span className="text-sm font-medium">
                    <span className="text-muted-foreground/75">
                      {new Date(return.createdAt).toLocaleDateString("en-US", {
                        year: "numeric",
                        month: "long",
                        day: "numeric",
                      })}
            ...
            <div className="flex flex-col justify-between h-full">
              <div className="flex items-center gap-2">
                {return.status && (
                  <Badge
                    color={
                      statusColors[return.status as keyof typeof statusColors] ||
                      "zinc"
                    }
              ...
              <div className="px-4 md:px-6 py-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                  <div>
                    ...
                    <span className="ml-2 font-medium">{return.id}</span>
                  </div>
                  {return.updatedAt && (
                    <div>
                      <span className="text-muted-foreground">Updated:</span>
                      <span className="ml-2 font-medium">
                        {new Date(return.updatedAt).toLocaleDateString()}
                      </span>
                    </div>
                  )}
                </div>
