/**
 * ReturnListPageClient - Client Component  
 * Based on dashboard ListPageClient but hardcoded for returns
 */
...
import { ReturnDetailsComponent } from '../components/ReturnDetailsComponent'
...
interface ReturnListPageClientProps {
  list: any
...
export function ReturnListPageClient({ 
  list, 
  initialData, 
  initialError, 
  initialSearchParams,
  statusCounts
}: ReturnListPageClientProps) {
  ...
  const breadcrumbs = [
    ...
    { type: 'page' as const, label: 'Returns' }
  ]
  ...
  return (
    <PageContainer title="Returns" header={header} breadcrumbs={breadcrumbs}>
      ...
      {error ? (
        ...
        <>
          ...
          <div className="grid grid-cols-1 divide-y">
            {data?.items?.map((return: any) => (
              <ReturnDetailsComponent key={return.id} return={return} list={list} />
            ))}
          </div>
