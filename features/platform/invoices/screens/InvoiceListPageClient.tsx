/**
 * InvoiceListPageClient - Client Component  
 * Based on dashboard ListPageClient but hardcoded for business accounts
 */
...
function EmptyStateDefault() {
  return (
    <EmptyState
      ...
      description="Customer business accounts will appear here when created."
...
export function InvoiceListPageClient({
  ...
  // For invoices, use raw GraphQL string to include relationship fields
  const querySelectedFields = `
    id accountNumber title description status totalAmount paidAmount creditLimit balanceDue
    formattedTotal formattedCurrentBalance formattedCreditLimit availableCredit
    currency { id code symbol }
    dueDate paidAt suspendedAt
    user { id name email }
    orders { id displayId total }
    createdAt updatedAt
  `
  ...
  const breadcrumbs = [
    ...
    { type: 'page' as const, label: 'Business Accounts' }
  ]
  ...
  const header = (
    <div>
      ...
      <p className="text-muted-foreground">
        Manage customer business accounts and billing
      </p>
    </div>
  )
  ...
  return (
    <PageContainer title="Business Accounts" header={header} breadcrumbs={breadcrumbs}>
        ...
        <>
          ...
          <div className="grid grid-cols-1 divide-y">
            {data?.items?.map((invoice: any) => (
              <InvoiceDetailsComponent key={invoice.id} invoice={invoice} list={list} />
            ))}
          </div>
          ...
          {data && data.count > pageSize && (
            <div className="px-4 md:px-6 py-4">
              <Pagination
                ...
                total={data.count}
                pageSize={pageSize}
                list={{ singular: 'account', plural: 'accounts' }}
              />
            </div>
          )}
        </>
