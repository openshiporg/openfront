/**
 * ProductCategoryListPageClient - Client Component  
 * Based on dashboard ListPageClient but hardcoded for productcategories
 */
...
interface ProductCategoryListPageClientProps {
  list: any
...
function EmptyStateDefault() {
  return (
    <EmptyState
      title="No Product Categories Created"
      description="You can create a new product category to get started."
...
export function ProductCategoryListPageClient({
  list,
  initialData,
  initialError,
  initialSearchParams,
  statusCounts
}: ProductCategoryListPageClientProps) {
  ...
  // For product-categories, use raw GraphQL string to include relationship fields (from working repomix)
      ...
      {error ? (
        ...
        <>
          ...
          <div className="grid grid-cols-1 divide-y">
            {data?.items?.map((productcategory: any) => (
              <ProductCategoryDetailsComponent key={productcategory.id} productcategory={productcategory} list={list} />
            ))}
          </div>
          ...
          {data && data.count > pageSize && (
            <div className="px-4 md:px-6 py-4">
              <Pagination
                ...
                total={data.count}
                pageSize={pageSize}
                list={{ singular: 'productcategory', plural: 'productcategories' }}
              />
            </div>
          )}
        </>
