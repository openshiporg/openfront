/**
 * CollectionListPageClient - Client Component  
 * Based on dashboard ListPageClient but hardcoded for collections
 */
...
interface CollectionListPageClientProps {
  list: any
...
function EmptyStateDefault() {
  return (
    <EmptyState
      ...
      description="You can create a new collection to get started."
...
export function CollectionListPageClient({
  ...
  // For product-collections, use raw GraphQL string to include relationship fields (from working repomix)
  ...
  const breadcrumbs = [
    ...
    { type: 'page' as const, label: 'Collections' }
  ]
      ...
      {error ? (
        ...
        <>
          ...
          <div className="grid grid-cols-1 divide-y">
            {data?.items?.map((collection: any) => (
              <CollectionDetailsComponent key={collection.id} collection={collection} list={list} />
            ))}
          </div>
          ...
          {data && data.count > pageSize && (
            <div className="px-4 md:px-6 py-4">
              <Pagination
                ...
                total={data.count}
                pageSize={pageSize}
                list={{ singular: 'collection', plural: 'collections' }}
              />
            </div>
          )}
        </>
