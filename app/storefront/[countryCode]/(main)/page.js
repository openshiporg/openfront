import { getCollectionsList, getProductsList, getRegion } from "@storefront/lib/data"
...
const getCollectionsWithProducts = cache(async countryCode => {
  ...
  await Promise.all(collectionIds.map((id) =>
    ...
    responses.forEach(({ response, queryParams }) => {
      ...
      if (collections) {
        collection = collections.find((collection) => collection.id === queryParams?.collection_id?.[0])
      }
