...
export function ShippingProviderRates({
  ...
  // Use a Set to record carrier keys that fail to load their icon from fallback (e.g., GitHub)
  const [failedIcons, setFailedIcons] = useState(new Set());
  // New state for pagination and sorting
  const [currentPage, setCurrentPage] = useState(1);
  const [sortBy, setSortBy] = useState("default");

  // Insert a useMemo block to derive sortedRates based on sortBy
  ...
  return (
    <div className="space-y-2">
      <div className="flex items-center justify-between">
        ...
        >
          <SelectTrigger className="p-0 border-none w-auto h-auto">
