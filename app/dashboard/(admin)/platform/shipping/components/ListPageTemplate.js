...
export function ListPageTemplate({ listKey = "ShippingOption" }) {
  ...
  const searchFields = ["name", "region", "provider"];
  const searchLabels = ["Name", "Region", "Provider"];
  ...
  // Custom field views for Shipping list
  const fieldViews = {
    name: {
      render: ({ item }) => (
        <div className="flex flex-col">
          ...
          {item.region && (
            <div className="text-sm text-muted-foreground">
              {item.region.name} ({item.region.currency.code})
            </div>
          )}
        </div>
      )
    ...
    provider: {
      render: ({ item }) => (
        <div className="flex items-center gap-2">
          <Badge variant={item.provider?.isEnabled ? "default" : "secondary"}>
        ...
        <div className="space-y-1">
          {item.requirements?.map(req => (
            <div key={req.id} className="text-sm">
              {req.type === 'min_subtotal' && `Min: ${req.minSubtotal}`}
              {req.type === 'max_subtotal' && `Max: ${req.maxSubtotal}`}
              {req.type === 'amount' && `Amount: ${req.amount}`}
            </div>
          ))}
        </div>
      )
        ...
        <div className="mt-4 flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              Manage shipping options and providers
            </p>
          </div>
          {showCreate && (
            <CreateButtonLink href={`/dashboard/platform/shipping/new`}>
