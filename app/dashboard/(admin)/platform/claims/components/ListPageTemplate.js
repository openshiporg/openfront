...
export function ListPageTemplate({ listKey = "Claim" }) {
  ...
  const searchFields = ["id", "order.displayId", "order.email"];
  const searchLabels = ["ID", "Order ID", "Email"];
  ...
  // Custom field views for Claims list
  const fieldViews = {
    status: {
      render: ({ item }) => (
        <Badge
    ...
    type: {
      render: ({ item }) => (
        <Badge variant={item.type === "refund" ? "default" : "secondary"}>
          {item.type}
        </Badge>
      )
    },
    order: {
      render: ({ item }) => (
        <div>
          <a
            href={`/dashboard/platform/orders/${item.order.id}`}
    ...
    claimItems: {
      render: ({ item }) => (
        <div className="space-y-1">
          {item.claimItems.map(claimItem => (
            <div key={claimItem.id} className="text-sm">
              {claimItem.quantity}x {claimItem.item.title}
              <span className="text-xs text-muted-foreground"> - {claimItem.reason.label}</span>
            </div>
          ))}
        </div>
      )
          ...
          <div className="flex flex-col sm:flex-row mt-2 mb-4 gap-4 justify-between">
            <div className="flex-col items-center">
              ...
              <p className="text-muted-foreground">
                Manage customer claims and disputes
              </p>
            </div>
