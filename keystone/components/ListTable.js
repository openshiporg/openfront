...
import { getRootGraphQLFieldsFromFieldController } from "../../../../admin-ui/utils";
...
export function ListTable({
  selectedFields, listKey, itemsGetter, count, sort, currentPage, pageSize, selectedItems, onSelectedItemsChange, orderableFields
}) {
  const list = useList(listKey);
  const { query } = useRouter();
  const shouldShowLinkIcon = !list.fields[selectedFields.keys().next().value]
    .views.Cell.supportsLinkTo;
  return (
              ...
              <TableHeaderCell key={path}>
                <Link
                  ...
                  href={{
                    query: {
                      ...query,
                      sortBy: sort?.field === path && sort.direction === "ASC"
                        ? `-${path}`
                        : path
                    }
                  }}
