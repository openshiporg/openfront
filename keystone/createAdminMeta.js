...
export function createAdminMeta(config, initialisedLists) {
  const { lists } = config
  const adminMetaRoot = {
    listsByKey: {},
    lists: [],
  ...
  for (const [listKey, list] of Object.entries(initialisedLists)) {
    ...
    const maximumPageSize = Math.min(
      ...
      list.graphql.types.findManyArgs.take.defaultValue ?? Infinity
    )
    ...
    adminMetaRoot.listsByKey[listKey] = {
      ...
      initialColumns,
      initialSort: listConfig.ui?.listView?.initialSort ?? null,
  ...
  for (const [listKey, list] of Object.entries(initialisedLists)) {
    ...
    for (const [fieldKey, field] of Object.entries(list.fields)) {
      ...
      if (
        ...
        omittedLists.includes(field.dbField.list)
      )
        continue
      if (Object.values(field.graphql.isEnabled).every(x => x === false))
        continue
      ...
      const isNonNull = ["read", "create", "update"].filter(
      ...
      const fieldMeta = {
        ...
        customViewsIndex:
          field.ui.views === null
            ...
            : (assertValidView(
                field.views,
                `lists.${listKey}.fields.${fieldKey}.ui.views`
              ),
    ...
    for (const fieldMetaRootVal of adminMetaRoot.listsByKey[key].fields) {
      ...
      try {
        fieldMetaRootVal.fieldMeta =
          list.fields[fieldMetaRootVal.path].getAdminMeta?.() ?? null
      } finally {
...
function normalizeIsOrderFilter(input, baseOrderFilterArgs) {
  ...
  return context =>
    input({ context, session: context.session, ...baseOrderFilterArgs })
}
