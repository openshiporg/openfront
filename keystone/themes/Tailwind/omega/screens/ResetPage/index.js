...
import { useReinitContext } from "@keystone/keystoneProvider";
...
export function ResetPage() {
  ...
  const onSubmit = async (event) => {
    ...
    try {
      if (mode === "reset") {
        const { redeemUserPasswordResetToken } = await keystoneClient.request(`
        ...
        if (redeemUserPasswordResetToken?.code) {
          setError(redeemUserPasswordResetToken.message);
        } else {
          setSuccess("Password has been reset. You can now sign in.");
          setTimeout(() => router.push(`${basePath}/signin`), 3000);
        }
      } else {
        const { sendUserPasswordResetLink } = await keystoneClient.request(`
